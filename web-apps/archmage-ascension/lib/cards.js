...
function makeSharedArchitecture(cx, cy, W, H, e, val, sc){
  ...
  tarot:   { pipTop: 26, pipSize: 16, labelBelow: true,  numSize: 32, numPos: 'top-circle', artScale: 1.28 } // circle medallion at top, large name at bottom
};
  ...
  const cx=W/2, cy=H*0.48, artR=W*0.29*(layout.artScale||1);
  ...
  const topNumeral = layout.numPos==='top'
    ...
    : isTarot
    ? `<circle cx="${cx}" cy="${topCircleY.toFixed(1)}" r="${(topCircleR*1.30).toFixed(1)}" fill="${e.bg2}" fill-opacity=".96"/>
...
<text x="${cx}" y="${(topCircleY+numSz*0.36).toFixed(1)}" font-family="Cinzel,serif" font-weight="700" font-size="${numSz}" fill="${e.b}" fill-opacity=".92" filter="${flt}" text-anchor="middle">${val}</text>`
  ...
  const medNum = layout.numPos==='medallion'
    ...
    : '';
  const label = isTarot && layout.labelBelow
    ? `<line x1="${(cx-30*sc).toFixed(1)}" y1="${(H-40*sc).toFixed(1)}" x2="${(cx+30*sc).toFixed(1)}" y2="${(H-40*sc).toFixed(1)}" stroke="${e.m}" stroke-opacity=".36" stroke-width="${(0.8*sc).toFixed(1)}"/>
...
<line x1="${(cx-22*sc).toFixed(1)}" y1="${(H-16*sc).toFixed(1)}" x2="${(cx+22*sc).toFixed(1)}" y2="${(H-16*sc).toFixed(1)}" stroke="${e.m}" stroke-opacity=".22" stroke-width="${(0.55*sc).toFixed(1)}"/>`
    : layout.labelBelow
  ...
  const suitGlyphTR = isTarot
    ? renderTinyCurrentGlyph(elem, W-pipX, pipY - pipSz*0.5, Math.max(5*sc, pipSz*0.62), sc)
    : '';
  const topLeftCorner = `<text x="${pipX}" y="${pipY}" font-family="Cinzel,serif" font-weight="700" font-size="${pipSz}" fill="${e.b}" fill-opacity="${cornerFillOpacity}" filter="${flt}" text-anchor="middle"${cornerStroke}>${val}</text>`
    + ((fx.cornerGlyph || isTarot) ? renderTinyCurrentGlyph(elem, pipX, cornerGlyphY, cornerGlyphSize, sc) : '');
