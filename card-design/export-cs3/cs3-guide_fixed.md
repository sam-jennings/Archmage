...
```bash
node export-cs3/export.js --connector parallelogram --art echo --layout classic --pdf --pdf
```

This writes **101 SVG files** (one per card) to `export-cs3/svg-cards/`, a manifest
to `export-cs3/export-manifest.json`, and — with `--pdf` — matching PDFs to
`export-cs3/pdf-cards/`. The SVGs keep a high-resolution internal viewBox, but their
`width` and `height` are now written in millimetres so PDF export has the correct
physical page size instead of being inferred from pixels/DPI.
...
| `--scale` | `3` | Internal artwork scale. Use 3 unless you need heavier SVGs. |
| `--pdf` | off | Also export one PDF per card. |
| `--trim-width-mm` | `62` | Final trim width. |
| `--trim-height-mm` | `88` | Final trim height. |
| `--bleed-mm` | `3` | Bleed added on all sides. Default PDF/SVG page is therefore 68 × 94 mm. |
| `--no-bleed` | off | Export exact 62 × 88 mm pages instead; ratio 0.704545. |
...
| Setting | Value |
|---|---|
| Trim card size | 62 × 88 mm |
| Bleed | 3 mm on all sides |
| Exported PDF/SVG page | 68 × 94 mm by default, or 62 × 88 mm with `--no-bleed` |
| Resolution | Vector PDF/SVG; raster equivalence at 300 dpi or higher |
